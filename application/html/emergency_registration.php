...
$page_name2 = "Emergency Registration";
$page_name3 = "";
$page_name4 = "";

//
if(!empty($_GET['export']) && $_GET['export']=='true'){
	$sql_query="select  R.emergency_refno,R.emergency_name,R.emergency_email,R.emergency_office,R.emergency_phone from ytoa_emergency as R"; 
	$resultset=Query($sql_query);	
		...
		$col_ls=array('Reference No','Emergency Name','E-Mail','Office No','Mobile No');
		$filename = "Emergency-Registrations.xls";
...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	header("location:emergency_registration.php?search=1");
}

if(isset($_SESSION['ser_keyword'])){ $sub_qur.=" and (emergency_name like '%".$_SESSION['ser_keyword']."%' 
	or emergency_refno like '%".$_SESSION['ser_keyword']."%' or emergency_email like '%".$_SESSION['ser_keyword']."%' or emergency_office like '%".$_SESSION['ser_keyword']."%' or emergency_phone like '%".$_SESSION['ser_keyword']."%')"; }

if(!empty($_GET['status'])) { $status_id=$_GET['status']; $sub_qur.=" and emergency_status=".$_GET['status']; } else { $status_id=""; }
include(RNSTM);
?>
