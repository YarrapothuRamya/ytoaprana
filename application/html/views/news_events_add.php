    ...
    <form action="" method="post" name="" id="" enctype="multipart/form-data">
       <table width="100%" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <span class="item">
            <select name="va_cat_id" id="va_cat_id" placeholder="Type" style="width: 310px" required>
               	<? // rns_drop_foreach($gall_cat,@$row['va_cat_id'],"Type");?>
            </select>
            </span>
            <div class="clear"></div>
        </div>-->  
          <div class="row">
         	  <span class="label">Title &amp; Image <span class="red">*</span></span>          
              <span class="item">
			    <input type="hidden" name="ne_path" value="<? if(!empty($_GET['id'])){ echo $row["ne_path"];}?>">
		  <div class="input_fields_wrap">
          <div><input type="text" name="ne_title[]"  value="<? if(!empty($_GET['id'])) echo $row['ne_title'];?>" placeholder="Enter Title" required> <input class="files" name="user_files[]" type="file" <? if(empty($_GET['id'])){?>  required <? }?> ><? if(!empty($_GET['id'])){?><a href="<?=$img_path?>" target="_blank"><img src="<?=$site_url.RNSIM?>view.png" width="16" height="16" border="0" alt="Click to View Image" title="Click to View Image"  align="absmiddle"/></a> <? }else{ ?><!--<button type="button" class="button-blue rns-add" id="subClone" title="Add New"><i class="fa fa-plus fa-lg"></i></button>--><? }?> 
		  ...
		  <div class="row">
         	    <span class="label"> <span class="red"></span></span>
