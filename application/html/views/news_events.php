  ...
  <div class="search_box">
   <span class="fl">
    <form method="post">      
      ...
      <input type="button" class="button-orange" value="Clear" onClick="window.location.href='news_events?t=<?=$_GET['t']?>&status=1&reset'"/>
    </form>
     </span>
     <span class="fr">
    ...
    <table width="100%" class="table table-bordered">
      ...
      <tbody>
		...
		if(!empty($_GET['start'])){ $i= $_GET['start']; } else { $i=0; }
            $dept=0;
			 $qur_sel="select * from bi_news where ".$sub_qur." and ne_cat_id=".$parent_id." order by ne_order_by DESC";
        ...
        <tr>
          ...
          <td align="right" class="hide"><? if($dept!=1){?>
            <? if(!empty($row['ne_order_by'])){?>
            <a href="<?=RNSFIFQ?>&move=up&id=<?=$row['ne_id'];?>" class="icn-link-green tooltip"><i class="fa fa-sort-up fa-lg"></i></a>
            <? }} if($dept!=$cnt){?>
            &nbsp;<a href="<?=RNSFIFQ?>&move=down&id=<?=$row['ne_id'];?>" class="icn-link-red tooltip"><i class="fa fa-sort-down fa-lg"></i></a>
          <? }?></td>
          <td align="center" nowrap="nowrap" class="actionicons">
          	<a href="<?=$type?>&id=<?=$row['ne_id'];?>" class="icn-link-green tooltip" title="Edit"><i class="fa fa-pencil-square fa-lg"></i></a>
            <a href="newsevents_view?id=<?=$row['ne_id'];?>" class="preview_rns icn-link-green tooltip" title="Preview"><i class="fa fa-search fa-lg"></i></a>
            <span id="st_div_<?=$row['ne_id']?>"><a id="<?=$status_arr;?>" class="rec_status <?=$st_class?> tooltip" title="Status"><i class="fa fa-bullseye fa-lg"></i></a></span> </td>
        </tr>
        <? }}else{?>
        <tr>
