<div class="tablecontent">
    ...
    <form action="" method="post" name="designation" id="designation">
		...
		<div class="row">
            ...
            <span class="item">
            <select name="dt_state_id" id="dt_state_id" class="sel_ls" placeholder="" style="width: 310px" required>
            <option value="">Select State</option>
                <?php
                    dropdown('bi_states', 'st_id,st_name', 'st_status=1 order by st_name', @$row['dt_state_id']);
                ?>
            </select>
            </span>
          <div class="clear"></div>
        ...
        <div class="row" id="ltitle">
            ...
            <span class="item">
        <input name="dt_name" id="dt_name" type="text" value="<? if(isset($_POST['dt_name'])) echo $_POST['dt_name']; else if(!empty($_GET['id'])) echo $row['dt_name'];?>" required></span>
            <div class="clear"></div>
        </div>
  <div class="row" id="ltitle">
            <span class="label"> Abbreviation <span class="red"></span></span><span class="item">
        <input name="dt_abbrev" id="dt_abbrev" type="text" value="<? if(isset($_POST['dt_abbrev'])) echo $_POST['dt_abbrev']; else if(!empty($_GET['id'])) echo $row['dt_abbrev'];?>" ></span>
            <div class="clear"></div>
        </div>
    ...
    <form method="post">
        ...
        <input type="button" class="button-orange" value="Clear" onClick="window.location.href='districts.php?reset'"/>
    <div class="clear"></div>
    </form>
    </div>
    <div id="tab-container" class="box tab-container">
        <table width="100%" class="table table-bordered">
            <thead>
                ...
                <th width="20%"align="left">Abbreviation&nbsp;</th>
            ...
            <?php
            $dept=0;
			$qur=Query("select * from bi_districts as D
			            left join bi_states as S on S.st_id=D.dt_state_id
						where ".$sub_qur." order by st_id desc");
            $cnt=NumRows($qur);
            if($cnt>0){
                while($row=Fetch($qur)){ $dept++;
				
				if($row['st_status']==1) { $cs=2;$sm="Inactivate"; $st_class="icn-link-green";} else { $cs=1;$sm="Activate";$st_class="icn-link-red"; }
           		$status_arr=$row['dt_id'].",".$cs.",".$sm.","."bi_districts,dt_";
            ?>
            <tr>
            <td align="center"><?=$dept;?></td>
            <td align="left"><?=$row['st_name']?></td>
            <td align="left"><?=$row['dt_name']?></td>
            <td align="left"><?=$row['dt_abbrev']?></td>
            ...
            <td class="actionicons" align="center"><a href="?id=<?=$row['dt_id'];?>&pg=add" class="icn-link-green tooltip" title="Edit"><i class="fa fa-pencil-square fa-lg"></i></a>
			<span id="st_div_<?=$row['dt_id']?>"><a id="<?=$status_arr;?>" class="rec_status <?=$st_class?> tooltip" title="Status"><i class="fa fa-bullseye fa-lg"></i></a></span> 
			</td>
            </tr>
            <? }}else{?>
            <tr>
