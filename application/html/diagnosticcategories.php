...
$page_name2 = "";
$page_name3 = "Diagnostic Category";
...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	if(!empty($_GET['id'])){ $cond.=" and diagnosticcat_id<>'".$_GET['id']."' ";}
		$cnt_qur = getdata("ytoa_diagnosticcat","count(*)","diagnosticcat_name='".$_POST['diagnosticcat_name']."'".$cond);
	if($cnt_qur>0){
		$errmsg = 'Name or Category Name already Existed..';
	}else{		
		
		if(!empty($_GET['id'])){
			$_POST['diagnosticcat_updated_by']= $ERP_Uid;
			$_POST['diagnosticcat_updated_date']= $now_time;

			$updated_by_name=getdata("va_employees","emp_fname","emp_id='".$ERP_Uid."'");

			update_Defined('ytoa_diagnosticcat', array_map('trim',$_POST),"diagnosticcat_id='".$getid."'");
			$msg="up";
			$pharmacat_name=$_POST['diagnosticcat_name'];
//  Mail
	   ...
	   //$rand_link=$rand."-".$id;
		...
		$message_user 	= "<table width='550'  border='0' align='center' cellpadding='3' cellspacing='0' class='borblue'>
					  ...
					  <tr>
						<td align='left'  class='textfont'><strong>Diagnostic Category Updated Successfully.</strong></td>
					  </tr>
					    <tr>
						<td align='left'  class='textfont'></td>
					  </tr>
					   <tr>
						<td align='left'  class='textfont'><strong>Diagnostic Category  Name : </strong>".$diagnosticcat_name."</td>
					  </tr>
					  <tr>
						<td align='left'  class='textfont'></td>
					  </tr>
			...
			$mail_user->Subject	= "Y to A Prana Health Care Private Limited - Diagnostic Category Updated Successfully";
			...
			$orderby=rns_max('ytoa_diagnosticcat','diagnosticcat_orderby');
			$_POST['diagnosticcat_orderby']= $orderby;
			$_POST['diagnosticcat_added_by']= $ERP_Uid;
			$_POST['diagnosticcat_added_date']= $now_time;
			$Added_by_name	= getdata("va_employees","emp_fname","emp_id='".$ERP_Uid."'");

			insert_Defined('ytoa_diagnosticcat', array_map('trim',$_POST));		
	   ...
	   //$rand_link=$rand."-".$id;
		...
		$message_user 	= "<table width='550'  border='0' align='center' cellpadding='3' cellspacing='0' class='borblue'>
					  ...
					  <tr>
						<td align='left'  class='textfont'><strong>Diagnostic Category Added Successfully.</strong></td>
					  </tr>
					    <tr>
						<td align='left'  class='textfont'></td>
					  </tr>
					   <tr>
						<td align='left'  class='textfont'><strong>Diagnostic Category Name : </strong>".$_POST['diagnosticcat_name']."</td>
					  </tr>
					   <tr>
						<td align='left'  class='textfont'><strong>Category Description : </strong>".$_POST['diagnosticcat_desc']."</td>
					  </tr>
					  <tr>
						<td align='left'  class='textfont'></td>
					  </tr>
			...
			$mail_user->AddBCC(DEV_MAIL,FROM_NAME);
			$mail_user->IsHTML(true);
			$mail_user->Subject	= "Y to A - Prana Health Care Private Limited - Diagnostic Category Added Successfully";
			$mail_user->Body		= stripslashes($mail_body_user);
			$mail_user->Send();
			//echo $mail_body_user; exit;
	...
	header("location:diagnosticcategories.php?search=1");
   }	
}
if(!empty($_GET['search'])) { 
	if(isset($_SESSION['ser_key'])){ $sub_qur.=" and (diagnosticcat_name like '%".$_SESSION['ser_key']."%')"; }
}
if(!empty($_GET['id'])){
	$qur = Query("select * from ytoa_diagnosticcat where diagnosticcat_id=".$_GET['id']);
	$row = Fetch($qur);	
}
include(RNSTM);
?>
