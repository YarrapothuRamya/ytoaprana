...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	if($cnt_qur>0){
		...
		if(!empty($_POST['privileges'])){ $emp_privileges="|DBORD|VIEW|"; $emp_privileges.=implode('|', $_POST['privileges']); }else{ $_POST['emp_privileges'] = ""; }unset($_POST['privileges']);
		if(!empty($emp_privileges)){  $_POST['emp_privileges'] = $emp_privileges; }
		...
		if(!empty($_GET['id'])){
			...
			$code=substr(getdata("va_employees","max(emp_code)",1),5);
			if($code!="") { $code=$code+1; } else {  $code=1; }
			$emp_code="SCI".rnsNumPad(4,$code);
