...
$page_name2 = "Pharma Registration";
$page_name3 = "";
$page_name4 = "";

//
if(!empty($_GET['export']) && $_GET['export']=='true'){
	$sql_query="select  R.pharma_refno,R.pharma_name,R.pharma_email,R.pharma_office,R.pharma_phone from ytoa_pharma as R"; 
	$resultset=Query($sql_query);	
		...
		$col_ls=array('Reference No','Pharma Name','E-Mail','Office No','Mobile No');
		$filename = "Pharma-Registrations.xls";
...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	header("location:pharma_registration.php?search=1");
}

if(isset($_SESSION['ser_keyword'])){ $sub_qur.=" and (pharma_name like '%".$_SESSION['ser_keyword']."%' 
	or pharma_refno like '%".$_SESSION['ser_keyword']."%' or pharma_email like '%".$_SESSION['ser_keyword']."%' or pharma_office like '%".$_SESSION['ser_keyword']."%' or pharma_phone like '%".$_SESSION['ser_keyword']."%')"; }

if(!empty($_GET['status'])) { $status_id=$_GET['status']; $sub_qur.=" and pharma_status=".$_GET['status']; } else { $status_id=""; }
include(RNSTM);
?>
