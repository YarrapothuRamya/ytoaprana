...
$page_name2 = "";
$page_name3 = "Pharma Sub-Category";
...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	if(!empty($_GET['id'])){ $cond.=" and pharmasubc_id<>'".$_GET['id']."' ";}
		$cnt_qur = getdata("ytoa_pharmasub_cat","count(*)","pharmasubc_name='".$_POST['pharmasubc_name']."' and pharmasubc_catid='".$_POST['pharmasubc_catid']."'".$cond);
	if($cnt_qur>0){
		$errmsg = 'Name or Pharma Sub-Category Name for this Category already Existed..';
	}else{		
		
		if(!empty($_GET['id'])){
			$_POST['pharmasubc_updated_by']= $ERP_Uid;
			$_POST['pharmasubc_updated_date']= $now_time;
			$updated_by_name=getdata("va_employees","emp_fname","emp_id='".$ERP_Uid."'");

			update_Defined('ytoa_pharmasub_cat', array_map('trim',$_POST),"pharmasubc_id='".$getid."'");

			$category	= getdata("ytoa_pharmacat","pharmacat_name","pharmacat_id='".$_POST['pharmasubc_catid']."'");
			...
			//$rand_link=$rand."-".$id;
			 ...
			 $message_user 	= "<table width='550'  border='0' align='center' cellpadding='3' cellspacing='0' class='borblue'>
						   ...
						   <tr>
							 <td align='left'  class='textfont'><strong>Pharma Sub-Category Updated Successfully.</strong></td>
						   </tr>
							 <tr>
							 <td align='left'  class='textfont'></td>
						   </tr>
						   ...
						   <tr>
							...
							<tr>
							 <td align='left'  class='textfont'><strong> Subcategory  Name : </strong>".$_POST['pharmasubc_name']."</td>
						   </tr>
						   <tr>
							 <td align='left'  class='textfont'></td>
						   </tr>
			...
			$orderby=rns_max('ytoa_pharmasub_cat','pharmasubc_orderby');
			$_POST['pharmasubc_orderby']= $orderby;
			$_POST['pharmasubc_added_by']= $ERP_Uid;
			$_POST['pharmasubc_added_date']= $now_time;
			insert_Defined('ytoa_pharmasub_cat', array_map('trim',$_POST));			
			...
			//$rand_link=$rand."-".$id;
			 ...
			 $message_user 	= "<table width='550'  border='0' align='center' cellpadding='3' cellspacing='0' class='borblue'>
						   ...
						   <tr>
							 <td align='left'  class='textfont'><strong>New Pharma Sub-Category Added successfully.</strong></td>
						   </tr>
							 <tr>
							 <td align='left'  class='textfont'></td>
						   </tr>
						   ...
						   <tr>
							...
							<tr>
							 <td align='left'  class='textfont'><strong> Sub-Category  Name : </strong>".$_POST['pharmasubc_name']."</td>
						   </tr>
						   <tr>
							 <td align='left'  class='textfont'></td>
						   </tr>
				 ...
				 $mail_user->AddBCC(DEV_MAIL,FROM_NAME);
				 $mail_user->IsHTML(true);
				 $mail_user->Subject	= "Y to A - Prana - Pharma Sub-Category Added Successfully.";
				 $mail_user->Body		= stripslashes($mail_body_user);
				 $mail_user->Send();
				 //echo $mail_body_user; exit;
	...
	//}
	header("location:pharmasub_categories.php?search=1");
   }	
}
if(!empty($_GET['search'])) { 
     if(isset($_SESSION['ser_cat_id'])){ $sub_qur.=" and pharmasubc_catid =".$_SESSION['ser_cat_id']; }
	if(isset($_SESSION['ser_key'])){ $sub_qur.=" and (pharmasubc_name like '%".$_SESSION['ser_key']."%')"; }
}
if(!empty($_GET['id'])){
	$qur = Query("select * from ytoa_pharmasub_cat where pharmasubc_id=".$_GET['id']);
	$row = Fetch($qur);	
}
include(RNSTM);
?>
