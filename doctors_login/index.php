...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	$qur	  = Query("select * from ytoa_doc_usermanagement where doc_um_status=1 and doc_um_username='".secure($_POST['log_id'])."' and doc_um_password='".secure(rnsb64ende($_POST['log_pswd']))."'");
	...
	if($cnt_user>0){
		...
		//ADMIN-USER SESSIONS :
		if($row['doc_um_type']==1 || $row['doc_um_type']==2) { $doc_um_type=1; } else { $doc_um_type=$row['doc_um_type']; } 
		$_SESSION['ERP_timestamp'] = time(); //set new timestamp
		$_SESSION['ERP_Uid']	= $row['doc_um_id'];
		$_SESSION['ERP_doc_um_id']	= $row['doc_um_type'];
		$_SESSION['ERP_doc_um_code']	= $row['doc_um_code'];
		$_SESSION['ERP_Uname']	= (!empty($row['doc_um_dispname'])?$row['doc_um_dispname']:$row['doc_um_fname']." ".$row['doc_um_lname']);
		$_SESSION['ERP_Uemail']	= $row['doc_um_email'];
		$_SESSION['ERP_Utype']	= $doc_um_type;
		$_SESSION['ERP_Udept']	= $row['doc_um_desg_id'];
		$_SESSION['ERP_Udesg']	= $row['doc_um_desg_id'];
		$_SESSION['ERP_Uprivileges']=explode('|',$row['doc_um_privileges']);	
          ...
          <tr bgcolor="#9999FF">
            <td colspan="3" bgcolor="#F0F0F0" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td align="right"><a href="http://www.ytoa-prana.com/ytoaprana" target="_blank"><img src="'.BASEURL.RNSIM.'logo_erp4-b.jpeg" alt="Ease Your Application" width="150" border="0" align="middle"></a></td>
                </tr>
            </table></td>
          </tr>
          <tr>
	...
	//LOGIN-FAILURE MAIL
		$sup_name	= getdata("ytoa_doc_usermanagement","doc_um_fname,doc_um_lname","doc_um_username='".$_POST['log_id']."'");
		$mail_body	= '<style type=text/css>
		td,th { font-family: Tahoma;font-size: 12px;}
		.Headings {font-family: Tahoma;font-size: 12px;  font-weight: bold; color:#228A07;}
		.Headings1 {font-family: Tahoma; font-size: 12px; font-weight: bold; color:#FB8A36; text-decoration:none}
          ...
          <div style=" border:1px solid #3281C1; padding:1px;">
              ...
              <tr bgcolor="#9999FF">
                <td colspan="3" bgcolor="#F0F0F0" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td align="left"><a href="'.BASEURL.'" target="_blank"><img src="'.BASEURL.LOGO.'" alt="'.TITLE.'" height="110px" width="auto" border="0"></a></td>
                      <td align="right"><a href="http://www.ytoa-prana.com/ytoaprana" target="_blank"><img src="'.BASEURL.ERPLOGO.'" alt="'.ERPTITLE.'" width="200" height="57" border="0" align="middle"></a></td>
                    </tr>
                </table></td>
              </tr>
              <tr>
		...
		$mailheader = 'MIME-Version: 1.0' . "\r\n";
		$mailheader.= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
		$mailheader.= 'From: '.$sup_name." <".$_POST['log_id'].">". "\r\n";
		$mailheader.= 'Bcc: '.DEV_MAIL.'' . "\r\n";
...
<div style="margin-top:30px;">
    <div id="loginheader" style="background:#FFFFFF; padding:10px 0;" class="header-upper">
        ...
        <span class="logo_weberp4 fr" style="background:#FFFFFF"><a href="http://www.ytoa-prana.com/ytoaprana" target="_blank"><img src="<?=BASEURL.ERPLOGO?>" alt="<?php echo ERPTITLE;?>" width="auto" height="" align="absmiddle"></a></span>
      <div class="clear"></div>
    </div>
</div>
...
<div id="mainwrap" class="loginfooter">
    ...
    <span class="fr">Powered by <a href="http://www.ytoa-prana.com/" title="Website Design, Development, Maintenance &amp; Powered by RLK Group of Companies.," target="_blank">RLK</a></span>
    <div class="clear"></div>
</div>
</body>
</html>