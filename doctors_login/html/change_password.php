...
if($_SERVER['REQUEST_METHOD']=="POST"){
	$errmsg="";
	$qur_emp=Query("select * from ytoa_doctor where doc_password = '".secure(rnsb64ende($_POST['pwd1']))."' and doc_id='".$_SESSION['ERP_Uid']."'");
	$cnt_emp=NumRows($qur_emp);
	$row_emp=Fetch($qur_emp);
	if($cnt_emp>0){		
		Query("UPDATE ytoa_doctor set doc_password = '".secure(rnsb64ende($_POST['pswd']))."' where doc_id='".$_SESSION['ERP_Uid']."' and doc_password='".secure(rnsb64ende($_POST['pwd1']))."'");	

		Query("UPDATE ytoa_doc_usermanagement set doc_um_password = '".secure(rnsb64ende($_POST['pswd']))."' where doc_um_reg_idfk='".$_SESSION['ERP_Uid']."' and doc_um_password='".secure(rnsb64ende($_POST['pwd1']))."'");	
				...
				<td align='left' valign='top'><table width='100%' border='0' align='center' cellpadding='5' cellspacing='0'>
					...
					<tr>
					  <td><table width='100%' border='0' cellspacing='1' cellpadding='1'>
						  ...
						  <tr>
							<td>Username : ".$row['sup_username']."</td>
						  </tr>
						  <tr>
...
$res_sel=Query("select * from bi_suppliers where sup_id='".$_SESSION['ERP_Uid']."'");
