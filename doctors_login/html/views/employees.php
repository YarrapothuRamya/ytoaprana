<div class="tablecontent">
    ...
    <table width="100%" class="table table-bordered" id="employee">
       <thead>
            ...
            $qur_sel="select * from bi_suppliers as E
			left join va_employees_type as ET on E.sup_type=ET.empt_id
			where ".$sub_qur." and sup_type<>1 order by sup_order_by desc";
            $qur_cnt=Query($qur_sel);
            $cnt_sel=NumRows($qur_cnt);            
            $qur_sel=Query($qur_sel." limit $start,$len");			
            if($cnt_sel>0){
                while($row_sel=Fetch($qur_sel)){ $i++;
                if(!empty($row_sel['sup_avatar'])){
                    $avatar=BASEURLF.RNSUP."sup_photos/".$row_sel['sup_avatar'];
                } else {
                    $avatar=BASEURLF.RNSUP."noavatar.png";					
                }
            if($row_sel['sup_status']==1) { $cs=2;$sm="Inactivate"; $st_class="icn-link-green";} else { $cs=1;$sm="Activate";$st_class="icn-link-red"; }
            $status_arr=$row_sel['sup_id'].",".$cs.",".$sm.","."bi_suppliers,sup_";
            ?>
            
            <tr>
            <td align="center"><?=$i;?></td>
            <td align="left"><img src="<?=$avatar?>" width="50px;"></td>
            <td align="left"><?=$row_sel['sup_code']."<br>".$row_sel['empt_type'];?></td>
            <td align="left"><?=$row_sel['sup_fname']." ".$row_sel['sup_lname']; ?></td>
            <td align="left"><?=$row_sel['sup_email']; ?><div class="subtitle"><?=$row_sel['sup_phone'];?></div></td>
            <td align="center"><?=date('d-m-Y', strtotime($row_sel['sup_added_date']));?></td>
            <td align="center" nowrap class="actionicons">
            <? if($_SESSION['ERP_Sup_id']==1 || $_SESSION['ERP_Sup_id']==2){ ?>  <a href="employee_add?id=<?=$row_sel['sup_id'];?>" class="icn-link-green tooltip" title="Edit"><i class="fa fa-pencil-square fa-lg"></i></a> <? } ?>
            <a href="employee_view?id=<?=$row_sel['sup_id'];?>" class="preview_rns icn-link-green tooltip" title="Preview"><i class="fa fa-search fa-lg"></i></a>
           <? if($_SESSION['ERP_Sup_id']==1 || $_SESSION['ERP_Sup_id']==2){ ?> <span id="st_div_<?=$row_sel['sup_id']?>"><a id="<?=$status_arr;?>" class="rec_status <?=$st_class?> tooltip" title="Status"><i class="fa fa-bullseye fa-lg"></i></a></span> <? } ?>  </td>
            </tr>
            <? }}else{?>
            <tr>
