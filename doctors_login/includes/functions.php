...
function bsm_dropdown($table, $fields, $cond='', $selected=null,$spcond=''){
    ...
    $DropDown = mysqli_query($db, "SELECT $fields FROM $table WHERE $cond");
	while($row = mysqli_fetch_array($DropDown)){	
		$dropdown .= '<option value="bsm_'.$row[0].'" '.((($row[0]==$selected)&&($spcond=="bsm")) ? 'selected' : '').'>'.$row[1]." Super Market - Address: ".$row[2].", ".$row[3].", ".$row[4].", ".$row[5].", ".$row[6].'</option>'."\n";
	}
	echo $dropdown;
}
...
function bshgs_dropdown($table, $fields, $cond='', $selected=null,$spcond=''){
	...
	$DropDown = mysqli_query($db, "SELECT $fields FROM $table WHERE $cond");
	while($row = mysqli_fetch_array($DropDown)){	
		$dropdown .= '<option value="bshgs_'.$row[0].'" '.((($row[0]==$selected)&&($spcond=="bshgs")) ? 'selected' : '').'>'.$row[1]." SHG Store - Address: ".$row[2].", ".$row[3].", ".$row[4].", ".$row[5].", ".$row[6].'</option>'."\n";
	}
	echo $dropdown;
}
...
function bns_dropdown($table, $fields, $cond='', $selected=null,$spcond=''){
    ...
    $DropDown = mysqli_query($db, "SELECT $fields FROM $table WHERE $cond");
	while($row = mysqli_fetch_array($DropDown)){	
		$dropdown .= '<option value="bns_'.$row[0].'" '.((($row[0]==$selected)&&($spcond=="bns")) ? 'selected' : '').'>'.$row[1]." Network Store - Address: ".$row[2].", ".$row[3].", ".$row[4].", ".$row[5].", ".$row[6].'</option>'."\n";
	}
	echo $dropdown;
}
...
function Get_RefCount($refno)
{
	$res = Query("select count(*) as cnt from bi_biba	where bib_referalno='".$refno."'");
	$vrow = Fetch($res);
	return $vrow['cnt'];
}

function Get_Levelscount($result)
{
	global $db;
		$disp=preg_split ("/\,/", $result);
		$sno=0;
...
//				echo "<br>Total Count: ".$count;
			...
			$sno_disp--;
			//echo "<br>Total Count: ".$sno_disp;
		return $result." ".$sno_disp;
}
...
function default_sidebar()
{
	...
	$customerdtls_class="class=''";
	$productdtls_class="class=''";
	$team_class="class=''";
	$order_class="class=''";
	$cart_class="class=''";
	$promotion_class="class=''";
	$storeprod_class="class=''";
}
...
function getuser_storedtls($username)
{
	global $db;
	
	$store_qur = Query("select * from bi_biba where bib_username=".$username);
	$store_dtls = NumRows($store_qur);
	$store_row = Fetch($store_qur);
	$storedtls=$store_row['bib_storetype'].",".$store_row['bib_storeid'];
	return $storedtls;
}
?>